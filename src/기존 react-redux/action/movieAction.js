...
//받아온 키 값을 노출되지 않게 만든다 ->루트에 .env파일을 만든다
//미들웨어는 함수가 함수를 리턴
//영화 데이터 가져오기
function getMovies() {
  return async (dispatch) => {
    try {
      dispatch({ type: "GET_MOVIE_REQUEST" }); //로딩전 던져줌
...
function getDetailMovies(id) {
  return async (dispatch) => {
    try {
      ...
      console.log("detailMovies의 data는?  ", detailMovies);
      dispatch({
...
/*
  API 호출하는 방법
  1.Fetch   / 2. Ajax  /  3. Axios-1 보다 더 많은 기능, 라이브러리 형태
